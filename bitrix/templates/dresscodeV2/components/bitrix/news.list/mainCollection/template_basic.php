...
?>
	<?if(!empty($arResult["ITEMS"])):?>
		<?$this->SetViewTarget("main_collection_view_content_tab");?><div class="item"><a href="#"><?=$arResult["NAME"]?></a></div><?$this->EndViewTarget();?>
		<div class="tab">
								...
								<?foreach($arResult["ITEMS"] as $ixd => $arElement):?>
									...
									<li class="item" id="<?=$this->GetEditAreaId($arElement["ID"]);?>">
										<div class="wrap">
											...
											<a href="<?=$arElement["DETAIL_PAGE_URL"]?>" class="more"><?=GetMessage("MAIN_COLLECTION_MORE")?></a>
										</div>
									</li>
								<?endforeach;?>
							</ul>
