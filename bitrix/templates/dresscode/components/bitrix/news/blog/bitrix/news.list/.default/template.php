				...
				<div class="tile<?if(!empty($startIndex)):?><?if(empty($arNextElement["RESIZE_PICTURE"])):?> no-image<?else:?> center-image<?endif;?><?endif;?>">
					<div class="tb">				
						<div class="tc">
							...
							<?endif;?>
							<div class="tile-text">
								<?if(!empty($arNextElement["ACTIVE_FROM"])):?>
									<div class="tile-date"><?=CIBlockFormatProperties::DateFormat($arParams["ACTIVE_DATE_FORMAT"], MakeTimeStamp($arNextElement["ACTIVE_FROM"], CSite::GetDateFormat()));?></div>
								<?endif;?>
