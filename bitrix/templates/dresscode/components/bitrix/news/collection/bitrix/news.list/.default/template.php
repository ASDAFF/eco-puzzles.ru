						...
						<?if(!empty($arColumn["ITEMS"])):?>
							<div class="items">
								<?foreach ($arColumn["ITEMS"] as $ix => $arElement):?>
									<?$image =  CFile::ResizeImageGet($arElement["DETAIL_PICTURE"], array("width" => 430, "height" => 250), BX_RESIZE_IMAGE_PROPORTIONAL, false);?>  
									<div class="item" id="<?=$this->GetEditAreaId($arElement["ID"]);?>">
										...
										<?if(!empty($arElement["DETAIL_TEXT"])):?>
											<div class="description"><?=$arElement["DETAIL_TEXT"]?></div>
										<?endif;?>
										<a href="<?=$arElement["DETAIL_PAGE_URL"]?>" class="more"><?=GetMessage("COLLECTION_MORE")?></a>
									</div>
								<?endforeach;?>
							</div>
						<?endif;?>
