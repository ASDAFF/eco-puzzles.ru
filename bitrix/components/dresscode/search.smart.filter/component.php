...
if(isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] == "y"){
	$_REQUEST["q"] = (BX_UTF != true) ? iconv("UTF-8", "WINDOWS-1251//ignore", $_REQUEST["q"]) : $_REQUEST["q"];
}

$SEARCH_QUERY = (string)trim($_REQUEST["q"]);
$FILTER_NAME = (string)$arParams["FILTER_NAME"];

$arLang = CSearchLanguage::GuessLanguage($SEARCH_QUERY);
...
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	$arAppendFilter["?NAME"] = $SEARCH_QUERY;
	$arAppendFilter["?PROPERTY_CML2_ARTICLE"] = $SEARCH_QUERY;

	foreach ($arResult["SEARCH_PROPERTIES"] as $index => $arNextProp){
		if($arNextProp["PROPERTY_TYPE"] == "L"){
			$arAppendFilter["?PROPERTY_".$arNextProp["CODE"]."_VALUE"] = $SEARCH_QUERY;
		}else{
			$arAppendFilter["?PROPERTY_".$arNextProp["CODE"]] = $SEARCH_QUERY;
		}
	}
	...
	if(!empty($arResult["ITEMS"]))
	{
		if ($this->facet->isValid())
		{
			...
			$arResult["FACET_FILTER"][] = $arAppendFilter;
		...
		else
		{
			...
			$arElementFilter[] = $arAppendFilter;
