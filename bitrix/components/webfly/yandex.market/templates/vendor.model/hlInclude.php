...
foreach ($arResult["OFFER"] as &$arOffer)
{
    ...
    foreach ($arParams as $k => $v)
    {
        ...
        {
            if (!empty($arOffer["DISPLAY_PROPERTIES"][$code]["USER_TYPE_SETTINGS"]["TABLE_NAME"]))
            {
                $tableName = $arOffer["DISPLAY_PROPERTIES"][$code]["USER_TYPE_SETTINGS"]["TABLE_NAME"];
                ...
                if (!empty($hlblock))
                {
                    ...
                    $prodColor = $entity_data_class::getList(array("select" => array("UF_NAME"), "filter" => array("UF_XML_ID" => $arOffer["DISPLAY_PROPERTIES"][$code]["VALUE"])))->fetch();
                    $arOffer["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"] = $prodColor["UF_NAME"];
                }
            }
        }
        ...
        if ($x == 0 && !empty($arOffer["GROUP_ID"]))
        {
            ...
            if (empty($arOffer["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]))
            {
                if (!empty($arOffer["DISPLAY_PROPERTIES"][$code]["USER_TYPE_SETTINGS"]["TABLE_NAME"]))
                {
                    ...
                    $hlbl1 = intval($res["id"]);
                    $hlblock1 = HL\HighloadBlockTable::getById($hlbl1)->fetch();
                    if (!empty($hlblock1))
                    {
                        $entity1 = HL\HighloadBlockTable::compileEntity($hlblock1);
                        $entity_data_class1 = $entity1->getDataClass();
                        $prodColor = $entity_data_class1::getList(array("select" => array("UF_NAME"), "filter" => array("UF_XML_ID" => $arOffer["DISPLAY_PROPERTIES"][$code]["VALUE"])))->fetch();
                        $arOffer["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"] = $prodColor["UF_NAME"];
                    }
                }
            }
