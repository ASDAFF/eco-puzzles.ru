...
<div id="skyweb24_contact_type3" style="<?if(!empty($arResult['GOOGLE_FONT'])){?>font-family:<?=$arResult['GOOGLE_FONT']?><?}?>">
	...
	<form action="<?=$templateFolder?>/ajax.php" method="POST" onsubmit="sendForm3<?=$arParams['ID_POPUP']?>(this);return false;">
	...
	<script>
		function sendForm3<?=$arParams['ID_POPUP']?>(f){
			var sendO={},
			...
			BX.ajax({
				...
				onsuccess: function(data){
					BX("skyweb24_contact_type3").outerHTML=data;
