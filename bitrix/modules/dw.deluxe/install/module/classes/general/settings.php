...
class DwSettings{
	...
	public function getCurrentSettings(){
		...
		if(!self::checkSettingsFile()){
			showError("settings.php - not found!");
			return false;			
		}
	...
	public function setSiteDir($siteDir){
		...
		if(!empty($siteDir)){
			return $_SESSION["SETTINGS_SITE_DIR"] = $siteDir;
		}
	...
	public function getSiteDir(){
		return (!empty($_SESSION["SETTINGS_SITE_DIR"]) ? $_SESSION["SETTINGS_SITE_DIR"] : (defined("DELUXE_SITE_DIR") ? DELUXE_SITE_DIR : (
			!empty(SITE_DIR) ? SITE_DIR : "/")
		));
	}
