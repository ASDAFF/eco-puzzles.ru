												...
												<?foreach (array_merge($arResult["SET_ITEMS"]["DEFAULT"], $arResult["SET_ITEMS"]["OTHER"]) as $i => $arElement):?>
													<?if($arElement["ENABLED"]):?>
														<?
															...
															$this->AddDeleteAction($arElement['ID'], $arElement['DELETE_LINK'], CIBlock::GetArrayByID($arElement["ELEMENT_INFO"]["IBLOCK_ID"], "ELEMENT_DELETE"), array());
														?>
														<li class="setElement setWindowElement<?if($arElement["TYPE"] === "OTHER"):?> disabled<?endif;?>" data-price="<?=$arElement["PRICE_DISCOUNT_VALUE"]?>" data-discount="<?=$arElement["PRICE_DISCOUNT_DIFFERENCE_VALUE"]?>" data-id="<?=$arElement["ID"]?>" data-quantity="<?=$arResult["BASKET_QUANTITY"][$arElement["ID"]]?>">
