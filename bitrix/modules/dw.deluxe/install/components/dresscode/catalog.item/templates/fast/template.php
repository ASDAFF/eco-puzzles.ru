			...
			<div class="appFastViewColumnContainer">
				...
				<div class="appFastViewInformationColumn">
					...
					<?if(!empty($arResult["PRICE"])):?>
						...
						<?if($arResult["CATALOG_AVAILABLE"] != "Y"):?>
							<?if($arResult["CATALOG_SUBSCRIBE"] == "Y"):?>
								<a href="#" class="addCart subscribe" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/subscribe.png" alt="" class="icon"><?=GetMessage("FAST_VIEW_SUBSCRIBE_LABEL")?></a>
							<?else:?>
								<a href="#" class="addCart disabled" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="" class="icon"><?=GetMessage("FAST_VIEW_ADDCART_LABEL")?></a>
							<?endif;?>
						<?else:?>
							<a href="#" class="addCart" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="" class="icon"><?=GetMessage("FAST_VIEW_ADDCART_LABEL")?></a>
						<?endif;?>
					<?else:?>
					...
					<div class="catalogQtyBlock">
			            <label class="label"><?=GetMessage("FAST_VIEW_QUANTITY_LABEL")?> </label> <a href="#" class="catalogMinus"></a><input type="text" class="catalogQty"<?if(!empty($arResult["PRICE"]["EXTENDED_PRICES"])):?> data-extended-price='<?=\Bitrix\Main\Web\Json::encode($arResult["PRICE"]["EXTENDED_PRICES"])?>'<?endif;?> value="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>" data-step="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>" data-max-quantity="<?=$arResult["CATALOG_QUANTITY"]?>" data-enable-trace="<?=(($arResult["CATALOG_QUANTITY_TRACE"] == "Y" && $arResult["CATALOG_CAN_BUY_ZERO"] == "N") ? "Y" : "N")?>" max="9999"><a href="#" class="catalogPlus"></a>
			        </div>
