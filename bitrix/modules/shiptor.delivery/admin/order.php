...
//$filter_delivery_service = array(39);

//pre($filter_delivery_service);
...
if (!empty($orderList) && is_array($orderList))
{
	...
	foreach ($orderList as $orderId => $arOrder)
	{
		...
		$idTmp .= "</td>
			<td><b><a href='/bitrix/admin/sale_order_view.php?ID=".$orderId.GetFilterParams("filter_")."&lang=".LANGUAGE_ID."' title='".Loc::getMessage("SALE_DETAIL_DESCR")."'>##ID##</a></b></td>";
		$idTmp .= "</tr>";
		...
		{
			if(CSaleOrder::CanUserUpdateOrder($orderId, $arUserGroups))
...
$arGroupActionsTmp = array(
	...
	$arGroupActionsTmp["export_shiptor"] = array(
			"action" => "exportData('shiptor')",
			"value" => "export_shiptor",
			"name" => 'Отправить в Shiptor'//str_replace("#EXP#", "CommerceML 2.0", Loc::getMessage("SOAN_EXPORT_2"))
		);
...
$APPLICATION->SetTitle(Loc::getMessage("SALE_SECTION_TITLE").' Shiptor');
...
<div class="adm-c-bigdatabar" id="bigdatabar">
	<div class="adm-c-bigdatabar-container">
		...
		<div class="adm-c-bigdatabar-content">
			<div class="adm-c-bigdatabar-line">
				<p>Отображены все заказы. Для работы только с заказами Shiptor необходимо установить соответствующую опцию в
					<a href="/bitrix/admin/settings.php?lang=ru&mid=shiptor.delivery"> настройках</a> модуля.</p>
			</div>
		</div>
		<div class="clb"></div>
	</div>
...
if(!$order_sum){ $order_sum = 'Какой-то текст, сообщающий о том, что ничего не выбрано или еще о чем-то...'; }
?>
