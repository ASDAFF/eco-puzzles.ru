...
<yml_catalog date="<?= $arResult["DATE"] ?>">
            ...
            <offers>
                <? foreach ($arResult["OFFER"] as $arOffer): ?>
                    <offer id="<?= $arOffer["ID"] ?>"
                        ...
                        <? endif ?>
                        <? if (!empty($arOffer["MODEL_DEFAULT"])): ?>
                            <model><?= $arOffer["MODEL_DEFAULT"] ?></model>
                        <? endif ?>
                        ...
                        <?foreach ($arParams["PARAMS"] as $k => $v):
                            $param = $arOffer["DISPLAY_PROPERTIES"][$v];
                            if ($param["DISPLAY_VALUE"] && !in_array($v, $arResult["FOR_DELETE"])):?>
                                <param name="<?= $param["DISPLAY_NAME"] ?>"<?= $param["HINT"] != ""?' unit="'.$param["HINT"].'"':''?>><?= $param["DISPLAY_VALUE"] ?></param>
                            <?endif;
                        endforeach;?>
                    </offer>
                <? endforeach; ?>
    ...
    if (!function_exists("itemsCycle"))
    {
        ...
        function itemsCycle(&$savedXML, $arResult, $arParams) {
            foreach ($arResult["OFFER"] as $arOffer):
                $savedXML .= '<offer id="' . $arOffer["ID"] . '"';
                if (!empty($arOffer["GROUP_ID"]) && $arParams["SKU_SHOW_GROUP_ID"] == 'Y')
                    $savedXML .= ' group_id="' . $arOffer["GROUP_ID"] . '"';
                $savedXML .= ' available="' . $arOffer["AVAIBLE"] . '">';
                ...
                if (!empty($arOffer["MODEL_DEFAULT"])):
                    $savedXML .= '<model>' . $arOffer["MODEL_DEFAULT"] . '</model>';
                endif;
                if (!empty($arOffer["VENDOR_CODE"])):
                ...
                foreach ($arParams["PARAMS"] as $k => $v):
                    if ($arOffer["DISPLAY_PROPERTIES"][$v]["DISPLAY_VALUE"] && !in_array($v, $arResult["FOR_DELETE"])):
                        $savedXML .= '<param name="' . $arOffer["DISPLAY_PROPERTIES"][$v]["DISPLAY_NAME"] . '"';
                        if ($arOffer["DISPLAY_PROPERTIES"][$v]["HINT"] != "") {
                            $savedXML .= ' unit="'.$arOffer["DISPLAY_PROPERTIES"][$v]["HINT"].'"';
                        }
                        $savedXML .= '>';
    ...
    if (!function_exists("xmlHeader"))
    {
        ...
        function xmlHeader(&$savedXML, $arResult, $arParams,$server_name) {
            ...
            if (!empty($arResult["CURRENCIES"])):
                foreach ($arResult["CURRENCIES"] as $k => $cur):
                    if (!empty($cur) && $cur != 'RUR' && (!empty($arResult["WF_AMOUNTS"][$cur]) || $cur == 'RUB')): $savedXML .= '<currency id="' . $cur . '"' . (($cur == 'RUB') ? ' rate="1"' : ' rate="' . $arResult["WF_AMOUNTS"][$cur] . '"') . '/>';
                    endif;
                endforeach;
            else:
                ...
                else
                {
                    ...
                    <script type="text/javascript">
                        BX.ready(function () {
                            ...
                            var webflyJsPage = <?=$arResult["WF_NUM"]?>;
                            <?endif?>
