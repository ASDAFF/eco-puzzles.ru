...
<yml_catalog date="<?= $arResult["DATE"] ?>">
        <shop>
            ...
            <offers>
                <? foreach ($arResult["OFFER"] as $arOffer): ?>
                    <offer type="book" id="<?= $arOffer["ID"] ?>" available="<?= $arOffer["AVAIBLE"] ?>"<? if (!empty($arOffer["BID"])): ?> bid="<?= $arOffer["BID"] ?>"<? endif ?><? if (!empty($arOffer["CBID"])): ?> cbid="<?= $arOffer["CBID"] ?>"<? endif ?><? if (!empty($arOffer["FEE"])): ?> fee="<?= $arOffer["FEE"] ?>"<? endif ?>>
                        <url><?= $arOffer["URL"] ?></url>
    ...
    if (!function_exists("itemsCycle"))
    {
        ...
        function itemsCycle(&$savedXML, $arResult, $arParams) {
            foreach ($arResult["OFFER"] as $arOffer):
                $savedXML .= '<offer type="book" id="' . $arOffer["ID"] . '" available="' . $arOffer["AVAIBLE"] . '"' . (!empty($arOffer["BID"]) ? ' bid="' . $arOffer["BID"] . '"' : '') . (!empty($arOffer["CBID"])? ' cbid="' . $arOffer["CBID"] . '"' : '') . (!empty($arOffer["FEE"]) ? ' fee="' . $arOffer["FEE"] . '"' : '') . '>';
    ...
    if (!empty($arParams["BIG_CATALOG_PROP"]) and $_SESSION["WFYM_FINISH"] != "Yes")
    {
        if ((($arResult["WF_CURR"] - $arParams["BIG_CATALOG_PROP"]) < $arResult["WF_FULL"]))
        {
            if ($arResult["WF_CURR"] < $arResult["WF_FULL"])
            {
                ...
                {
                    itemsCycle($savedXML, $arResult, $arParams,$server_name);
