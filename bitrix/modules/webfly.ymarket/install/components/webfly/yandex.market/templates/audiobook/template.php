...
<yml_catalog date="<?= $arResult["DATE"] ?>">
        <shop>
            ...
            <offers>
                <? foreach ($arResult["OFFER"] as $arOffer): ?>
                    <offer type="audiobook" id="<?= $arOffer["ID"] ?>" available="<?= $arOffer["AVAIBLE"] ?>"<? if (!empty($arOffer["BID"])): ?> bid="<?= $arOffer["BID"] ?>"<? endif ?><? if (!empty($arOffer["CBID"])): ?> cbid="<?= $arOffer["CBID"] ?>"<? endif ?><? if (!empty($arOffer["FEE"])): ?> fee="<?= $arOffer["FEE"] ?>"<? endif ?>>
                        <url><?= $arOffer["URL"] ?></url>
    ...
    if (!function_exists("itemsCycle"))
    {
        ...
        function itemsCycle(&$savedXML, $arResult, $arParams) {
            foreach ($arResult["OFFER"] as $arOffer):
                $savedXML .= '<offer type="audiobook" id="' . $arOffer["ID"] . '" available="' . $arOffer["AVAIBLE"] . '"' . (!empty($arOffer["BID"]) ? ' bid="' . $arOffer["BID"] . '"' : '') . (!empty($arOffer["CBID"]) ? ' cbid="' . $arOffer["CBID"] . '"' : '') . (!empty($arOffer["FEE"]) ? ' fee="' . $arOffer["FEE"] . '"' : '') . '>';
